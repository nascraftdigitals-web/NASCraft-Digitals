...
export default function Forum() {
  ...
  const [posts, setPosts] = useState(() => {
    try {
      const saved = localStorage.getItem('nascraft_forum_posts')
  ...
  const savePosts = (updated) => {
    setPosts(updated)
    try { localStorage.setItem('nascraft_forum_posts', JSON.stringify(updated)) } catch {}
  }

  const submitPost = () => {
    if (!newPost.title || !newPost.body || !newPost.author) return
    savePosts([{ ...newPost, id: Date.now(), time:'Just now', likes:0, views:0, replies:[], tag:'teal' }, ...posts])
  ...
  const submitReply = () => {
    ...
    savePosts(posts.map(p => p.id === activePost ? { ...p, replies:[...p.replies,{author:'You',body:replyText,time:'Just now',isTeam:false}] } : p))
    setReplyText('')
  }

  const toggleLike = (id) => {
    setLiked(prev => ({ ...prev, [id]: !prev[id] }))
    savePosts(posts.map(p => p.id === id ? { ...p, likes: p.likes + (liked[id] ? -1 : 1) } : p))
  }
